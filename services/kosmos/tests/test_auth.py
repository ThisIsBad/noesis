"""Wiring smoke test — Kosmos uses the shared bearer-token gate.

Deep behaviour is covered in ``clients/tests/test_auth.py``. This file
pins that Kosmos wires the shared helper correctly.
"""
...
def _build_app(env: dict[str, str], monkeypatch) -> Starlette:
    for name in ("KOSMOS_SECRET", "KOSMOS_SECRET_PREV"):
        monkeypatch.delenv(name, raising=False)
    for name, value in env.items():
        monkeypatch.setenv(name, value)
    app = _downstream()
    app.add_middleware(bearer_middleware("KOSMOS_SECRET"))
    return app


def test_health_bypasses_auth_even_when_secret_is_set(monkeypatch):
    app = _build_app({"KOSMOS_SECRET": "test-secret"}, monkeypatch)
...
def test_rotation_accepts_previous_token(monkeypatch):
    app = _build_app(
        {"KOSMOS_SECRET": "new-token", "KOSMOS_SECRET_PREV": "old-token"},
        monkeypatch,
    )
    client = TestClient(app)
...
def test_server_module_imports_the_shared_middleware() -> None:
    import kosmos.mcp_server_http as server
