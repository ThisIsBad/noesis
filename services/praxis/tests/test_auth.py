"""Wiring smoke test — Praxis uses the shared bearer-token gate.
...
def _build_app(env: dict[str, str], monkeypatch) -> Starlette:
    for name in ("PRAXIS_SECRET", "PRAXIS_SECRET_PREV"):
...
def test_rotation_accepts_previous_token(monkeypatch):
    app = _build_app(
        {"PRAXIS_SECRET": "new-token", "PRAXIS_SECRET_PREV": "old-token"},
        monkeypatch,
    )
    client = TestClient(app)
