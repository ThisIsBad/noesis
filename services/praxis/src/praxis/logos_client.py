...
Praxis builds plans; Logos verifies them. Routing every plan check
through Claude as orchestrator costs 4× round trips per ``verify_plan``
call. The Logos-as-sidecar exception (see ``docs/ROADMAP.md``
``Kommunikations-Pattern``) lets read-only, idempotent verification
calls bypass Claude — Praxis itself can ask Logos "is this plan
goal-coverage-complete?" and stamp the returned ``ProofCertificate``
directly onto the plan.

This module is pure infrastructure: it adds the call path. Wiring
``PraxisCore.verify_plan`` to *use* it (auto-certifying plans before
``commit_step``) is a follow-up PR so the surface stays reviewable.
The same module shape lives in ``mneme.logos_client`` (PR #64); they
will likely consolidate into a shared package once a third caller
(Telos) lands.
...
from Praxis's perspective — losing a verification chance must not
prevent the underlying planning operation from succeeding. The
...
log = logging.getLogger("praxis.logos_client")
...
def _is_retryable(exc: BaseException) -> bool:
    ...
    here — services/praxis already pulls in httpx, but the classifier
