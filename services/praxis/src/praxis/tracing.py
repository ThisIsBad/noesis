"""Praxis tracing wiring for the Kairos observability service.
...
``PRAXIS_TRACE_ENABLED=0``, spans still run (timing + contextvars) but
...
def get_tracer() -> KairosClient:
    ...
    global _client
    if _client is None:
        enabled = _env_truthy("PRAXIS_TRACE_ENABLED")
