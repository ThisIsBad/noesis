"""Wiring smoke test — Hegemonikon uses the shared bearer-token gate.
...
``clients/tests/test_auth.py``; this file just pins that Hegemonikon
wires the helper up correctly with the right exempt paths so a
regression in the wiring trips Hegemonikon's own CI.
"""
...
def _build_app(env: dict[str, str], monkeypatch) -> Starlette:
    for name in ("HEGEMONIKON_SECRET", "HEGEMONIKON_SECRET_PREV"):
...
def test_health_bypasses_auth_even_when_secret_is_set(monkeypatch):
    app = _build_app({"HEGEMONIKON_SECRET": "test-secret"}, monkeypatch)
...
def test_rotation_accepts_previous_token(monkeypatch):
    app = _build_app(
        {"HEGEMONIKON_SECRET": "new-token", "HEGEMONIKON_SECRET_PREV": "old-token"},
        monkeypatch,
    )
    client = TestClient(app)
...
def test_static_prefix_is_public(monkeypatch):
    ...
    monkeypatch.setenv("HEGEMONIKON_SECRET", "tok")
    app2.add_middleware(
        bearer_middleware(
            "HEGEMONIKON_SECRET",
            exempt_paths={"/health"},
...
def test_server_module_imports_the_shared_middleware() -> None:
    """Sanity — Hegemonikon doesn't ship its own bearer auth class."""
    import hegemonikon.mcp_server_http as server
