...
from noesis_schemas import Prediction, CalibrationReport
...
class EpistemeCore:
    ...
    def log_prediction(self, claim: str, confidence: float, domain: Optional[str] = None) -> Prediction:
    ...
    def get_calibration(self, domain: Optional[str] = None) -> CalibrationReport:
        ...
        if not resolved:
            return CalibrationReport(domain=domain, sample_size=0, ece=0.0, brier_score=0.0, bias=0.0, sharpness=0.0)

        n = len(resolved)
        brier = sum((p.confidence - (1.0 if p.correct else 0.0)) ** 2 for p in resolved) / n
        ...
        # Simplified ECE: single bucket
        accuracy = sum(1 for p in resolved if p.correct) / n
        avg_conf = sum(p.confidence for p in resolved) / n
        ece = abs(avg_conf - accuracy)

        return CalibrationReport(domain=domain, sample_size=n, ece=ece, brier_score=brier, bias=bias, sharpness=sharpness)

    def should_escalate(self, confidence: float, domain: Optional[str] = None) -> bool:
        report = self.get_calibration(domain)
        # Escalate if confidence is low or calibration shows systematic overconfidence
