"""Wiring smoke test — Episteme uses the shared bearer-token gate.

Deep behaviour is covered in ``clients/tests/test_auth.py``. This file
pins that Episteme wires the shared helper correctly.
"""
...
def _build_app(env: dict[str, str], monkeypatch) -> Starlette:
    for name in ("EPISTEME_SECRET", "EPISTEME_SECRET_PREV"):
...
def test_health_bypasses_auth_even_when_secret_is_set(monkeypatch):
    app = _build_app({"EPISTEME_SECRET": "test-secret"}, monkeypatch)
...
def test_rotation_accepts_previous_token(monkeypatch):
    app = _build_app(
        {
            "EPISTEME_SECRET": "new-token",
            "EPISTEME_SECRET_PREV": "old-token",
        },
        monkeypatch,
    )
    client = TestClient(app)
